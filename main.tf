...
resource "aws_subnet" "dep5_pub_subnet1" {
  vpc_id                  = aws_vpc.dep5_main.id
...
resource "aws_subnet" "dep5_pub_subnet2" {
  vpc_id                  = aws_vpc.dep5_main.id
...
resource "aws_route_table" "dep5_rte_tbl" {
  vpc_id = aws_vpc.dep5_main.id

  route {
    cidr_block = var.route_table_cidr_block
    gateway_id = aws_internet_gateway.dep5_igw.id # associates route table with igw
  }

  tags = {
    dep5_route_table = var.dep5_route_table
  }
}

resource "aws_route_table_association" "pub_subnet_1" {
  subnet_id      = aws_subnet.dep5_pub_subnet1.id
  route_table_id = aws_route_table.dep5_rte_tbl.id
}

resource "aws_route_table_association" "pub_subnet_2" {
  subnet_id      = aws_subnet.dep5_pub_subnet2.id
  route_table_id = aws_route_table.dep5_rte_tbl.id
}
...
resource "aws_instance" "dep5_app_server" {

  depends_on = [aws_internet_gateway.dep5_igw]
  ...
  subnet_id                   = aws_subnet.dep5_pub_subnet1.id
  vpc_security_group_ids      = [aws_security_group.dep5_allow_traffic_jenkins.id]

  user_data = file("jenkins_installation.sh")

  tags = {
    "web_server" : var.app_server
  }
}

# configure dep5_app_server instance
resource "aws_instance" "dep5_web_server" {
  depends_on = [aws_internet_gateway.dep5_igw]
  ...
  key_name                    = var.key_name
  subnet_id                   = aws_subnet.dep5_pub_subnet2.id
  vpc_security_group_ids      = [aws_security_group.dep5_allow_web_traffic.id]

  user_data = file("python_installation.sh")

  tags = {
    "app_server" : var.web_server
  }
}
...
output "instance_ips" {
  ...
  value = {
    instance1 = aws_instance.dep5_web_server.id,
    instance2 = aws_instance.dep5_app_server.id
  }
}

# configure security group
# based on https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/security_group
resource "aws_security_group" "dep5_allow_traffic_jenkins" {
  vpc_id = aws_vpc.dep5_main.id
...
resource "aws_security_group" "dep5_allow_web_traffic" {
  vpc_id = aws_vpc.dep5_main.id
